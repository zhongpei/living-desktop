...
public struct MLXSampling: Sendable, Equatable {
...
public struct MLXGenerationResult: Sendable, Equatable {
    public var text: String
    public var usedFullPrefill: Bool
    public var prefixTokenCount: Int
    public var cacheHash: String
}

public enum MLXPromptCacheIdentity {
    public static var cacheDirectory: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("MyPet/BrainCaches", isDirectory: true)
    }

    public static func hash8(_ value: String) -> String {
    ...
    public static func fileURL(petID: String, cacheKey: String, promptVersion: Int) -> URL {
        cacheDirectory.appendingPathComponent(
            "\(petID)_v\(promptVersion)_\(hash8(cacheKey)).safetensors")
    }
}

/// Owns all MLX and tokenizer objects. The app supplies plain prompt values;
/// no game type crosses this boundary and no model result can mutate runtime
/// state directly.
public actor MLXGenerationRuntime {
    private var container: ModelContainer?
    private var tokenizer: (any Tokenizers.Tokenizer)?
    private var states: [String: BrainCacheManager.PromptState] = [:]
    ...
    public func generate(
        ...
        try Task.checkCancellation()
        guard let container, let tokenizer else { throw MLXGenerationError.modelUnavailable }

        let key = BrainCacheManager.Key(composite: cacheKey, promptVersion: promptVersion)
        let stateKey = "\(petID)/\(cacheKey)"
        let state: BrainCacheManager.PromptState
        if let existing = states[stateKey] {
            state = existing
            stateOrder.removeAll { $0 == stateKey }
            stateOrder.append(stateKey)
        } else {
            try Task.checkCancellation()
            state = try await BrainCacheManager.makePromptCache(
                container: container, tokenizer: tokenizer, petID: petID,
                key: key, messages: prefixMessages, diskEntries: diskEntries)
            states[stateKey] = state
            stateOrder.append(stateKey)
            trimMemory(to: memoryEntries)
        }
        try Task.checkCancellation()
        let input = try BrainCacheManager.decisionInput(
            state: state, tokenizer: tokenizer, dynamicText: dynamicText)
        let text = try await BrainCacheManager.generateText(
            container: container, cache: input.cache, input: input.input, sampling: sampling)
        try Task.checkCancellation()
        return MLXGenerationResult(
            text: text, usedFullPrefill: input.fullPrefill,
            prefixTokenCount: state.prefixTokens.count, cacheHash: key.hash8)
    }

    public func pruneDiskCache(maxEntries: Int) {
        BrainCacheManager.pruneDiskCache(maxEntries: maxEntries)
    }

    private func trimMemory(to capacity: Int) {
        while stateOrder.count > max(0, capacity), let oldest = stateOrder.first {
...
public enum MLXGenerationError: LocalizedError {
    case modelUnavailable

    public var errorDescription: String? { "MLX model or tokenizer is unavailable" }
}
...
enum BrainCacheManager {
    ...
    static func fileURL(petID: String, key: Key) -> URL {
        MLXPromptCacheIdentity.fileURL(
            petID: petID, cacheKey: key.composite, promptVersion: key.promptVersion)
    }
    ...
    static func prefill(container: ModelContainer, tokens: [Int]) async throws -> [any KVCache] {
        ...
        let box = try await container.perform { ctx -> UncheckedBox<[any KVCache]> in
            ...
            let parameters = GenerateParameters(prefill: .init(stepSize: 512))
    ...
    static func generateText(container: ModelContainer, cache: [any KVCache]?,
                             input: LMInput, sampling: MLXSampling) async throws -> String {
