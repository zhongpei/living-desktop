...
final class ClipLibrary {
    ...
    enum BaseMotion: String, CaseIterable {
        case idle, walk, run, airborne, drag
    }

    /// 素材像素实况朝向。down（正面）按 right 处理（正面镜像近似无损）。
    enum AuthoredFacing: String {
        case right, left, down
    }

    /// 是否需要水平镜像：素材实况朝左，或（向左移动时的）默认右向素材。
    /// movingRight = 宠物正朝右移动。
    static func mirrorNeeded(authored: AuthoredFacing, movingRight: Bool) -> Bool {
        switch authored {
    ...
    func propScaleOverride(for id: String) -> Double? {
    ...
    init(characterID: String, cellSize: CGSize, cacheLimit: Int = 24) {
    ...
    static func load(from dir: URL, cacheLimit: Int = 24) throws -> ClipLibrary {
    ...
    func clip(_ key: String) -> Clip? { clips[key] }
    func meta(for key: String) -> PetPackManifest.ClipMeta? { clips[key]?.meta }

    func facing(for key: String) -> AuthoredFacing {
        clips[key]?.meta.facing.flatMap(AuthoredFacing.init(rawValue:)) ?? .right
    }

    func playback(for key: String) -> Playback {
        clips[key]?.meta.playback.flatMap(Playback.init(rawValue:)) ?? .loop
    }

    /// 规范槽位的 clip 键（"base/walk"），无则 nil。
    func base(_ motion: BaseMotion) -> String? {
        clips["base/\(motion.rawValue)"] != nil ? "base/\(motion.rawValue)" : nil
    }

    /// 规范槽位 + 回退链：run 缺 → walk；airborne/drag 缺 → idle。保证身体线永远有帧。
    func baseOrFallback(_ motion: BaseMotion) -> String {
    ...
    func frames(for key: String) -> [CGImage]? {
    ...
    var debugCachedClipCount: Int { decodedCache.count }
...
enum PetPackError: LocalizedError {
    case empty(dir: URL)
...
enum PetPackLibrary {
    ...
    static func requestedPet(from arguments: [String] = CommandLine.arguments) -> String? {
        guard let i = arguments.firstIndex(of: "-pet"), i + 1 < arguments.count else { return nil }
        return arguments[i + 1]
    }

    /// 枚举库根下的宠物包（子目录含 manifest.json），按 id 字母序。
    static func availablePacks(in root: URL) -> [(id: String, url: URL)] {
    ...
    static func roots(bundle: Bundle = .main,
