...
final class SensesStore {
    ...
    func shouldResense(now: Double) -> Bool {
        isDirty && now - dirtyAt >= resenseCooldown
    }

    /// 注入大脑快照的 senses 段；过期或禁用返回 nil。
    func sensesSection(now: Double) -> String? {
